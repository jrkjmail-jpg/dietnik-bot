...
BOT_RELEASE=2026.06.13-single-month-1
...
- Постоянное хранилище:
  ...
  - `DB_PATH=/app/data/bot.db`
  - `PERSISTENCE_PATH=/app/data/bot_state.pkl`
- Для оплаты: добавь `YOOKASSA_SHOP_ID`, `YOOKASSA_SECRET_KEY`,
  `YOOKASSA_RETURN_URL`, цены и параметры чека
- Для админки: добавь `ADMIN_IDS`, например `123456789` или несколько ID через запятую
- Для ежедневного автобэкапа: `AUTO_DB_BACKUP_INTERVAL_HOURS=24`
- Для автоматической проверки оплаты: `PAYMENT_CHECK_INTERVAL_SECONDS=10`

Автобэкап выполняется не чаще одного раза в сутки. Значение
`AUTO_DB_BACKUP_INTERVAL_HOURS=0` полностью отключает автоматическую отправку базы.
...
- `/version` — показать версию реально запущенной сборки
...
Если у пользователя уже активен Basic, Premium оформляется как повышение тарифа.
Бот считает доплату пропорционально оставшимся дням:

```text
(цена Premium − цена Basic) × оставшиеся дни / 30
```

Сумма округляется вверх до целого рубля. После оплаты Premium действует до той
же даты, до которой был оплачен Basic. Если Basic выдан или оплачен больше чем
на 30 дней вперёд, доплата всё равно не превышает полную разницу тарифов —
`PREMIUM_PRICE_RUB − BASIC_PRICE_RUB`, то есть 400 ₽ при текущих ценах.
В сообщениях, закрывающих Premium-функции, бот показывает только кнопку Premium;
повторно купить Basic там нельзя.

Одинаковые тарифы нельзя покупать на несколько месяцев вперёд. Кнопка ручного
продления Basic или Premium появляется только за 3 дня до окончания. Продление
добавляет ровно 30 дней к текущей дате окончания, поэтому максимальный оплаченный
срок после продления составляет 33 дня. Автоплатёж пока не подключён: для него
нужны отдельное согласие пользователя и сохранение способа оплаты в ЮKassa.

Стоимость меняется переменными `BASIC_PRICE_RUB` и `PREMIUM_PRICE_RUB`.
Бот создаёт платёж напрямую через API ЮKassa и отправляет пользователю безопасную
ссылку на страницу оплаты. После подтверждения бот автоматически замечает оплату,
выдаёт доступ на 30 дней и отправляет сообщение об успешной активации.
Повторная обработка одного платежа не продлевает подписку второй раз.

### Подключение ЮKassa

BotFather Payments для этой интеграции не используется.

1. Открой личный кабинет ЮKassa и создай тестовый магазин.
2. Скопируй тестовые `shopId` и `secretKey`.
3. Добавь в BotHost:

```env
YOOKASSA_SHOP_ID=тестовый_shopId
YOOKASSA_SECRET_KEY=тестовый_secretKey
YOOKASSA_RETURN_URL=https://t.me/username_бота
YOOKASSA_TEST_MODE=1
BASIC_PRICE_RUB=490
PREMIUM_PRICE_RUB=890
...
4. Сделай редеплой и проверь покупку из раздела «Подписка».

После выбора Basic или Premium бот сначала спрашивает email для электронного
чека, создаёт платёж через API ЮKassa и отправляет кнопку «Оплатить».
Дальше фоновый контроллер сам запрашивает статус платежа у ЮKassa, сверяет
ID заказа, пользователя, тариф, сумму и валюту. При статусе `succeeded` бот
активирует подписку и сразу пишет пользователю срок действия тарифа.

`PAYMENT_CHECK_INTERVAL_SECONDS=10` означает, что подтверждение обычно приходит
в течение нескольких секунд после оплаты. Минимальное допустимое значение — 5.

`YOOKASSA_VAT_CODE=1` означает «без НДС». Если у магазина другая налоговая
настройка, укажи код из настроек ЮKassa. Параметры чека нужно согласовать с
настройками магазина или бухгалтером.

Для диагностики используй админ-команду `/paymentstatus` или `/paystatus`: она
показывает последние попытки создания платежей и точный ответ ЮKassa.

`YOOKASSA_TEST_MODE=1` только добавляет заметную пометку в сообщениях бота.
Тестовый или боевой режим фактически определяется ключами выбранного магазина.

Текущая версия использует разовые платежи без автоматического продления.
Для рекуррентных платежей потребуется отдельно сохранять способ оплаты и
оформить соответствующий сценарий в ЮKassa.
...
После редеплоя проверь в личном чате с ботом:

```text
/version
```

Бот должен показать `2026.06.13-single-month-1`. Если показывает старую версию или
