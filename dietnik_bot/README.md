...
BOT_RELEASE=2026.06.09-yookassa-api-1
...
ссылку на страницу оплаты. После подтверждения доступ выдаётся на 30 дней.
Повторная проверка одного платежа не продлевает подписку второй раз.
...
чека, создаёт платёж через API ЮKassa и отправляет кнопки «Оплатить» и
«Проверить оплату». При проверке бот повторно запрашивает платёж у ЮKassa и
сверяет ID заказа, пользователя, тариф, сумму и валюту.
...
Бот должен показать `2026.06.09-yookassa-api-1`. Если показывает старую версию или
