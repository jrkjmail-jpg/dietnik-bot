...
BOT_RELEASE=2026.06.09-payments-2
...
чека, затем отправляет стандартный счёт Telegram Payments с кнопкой оплаты
ЮKassa. Каждый счёт получает уникальный `payload`, поэтому бот проверяет
пользователя, тариф и сумму перед оплатой. `YOOKASSA_VAT_CODE=1` означает
«без НДС»; если у магазина другая
налоговая настройка, укажи код из настроек ЮKassa.

Для диагностики используй админ-команду `/paymentstatus`: она показывает
последние попытки создания счетов и точный ответ Telegram/ЮKassa.

Важно: Basic и Premium открывают цифровые функции внутри Telegram. По актуальным
правилам Telegram такие товары должны оплачиваться в Telegram Stars. Счёт
ЮKassa в RUB может быть отклонён платформой даже при правильном токене.
...
Бот должен показать `2026.06.09-payments-2`. Если показывает старую версию или
